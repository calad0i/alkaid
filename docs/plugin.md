...
- **Model Tracer Class**: This class is responsible for tracing the model, or any generic dataflow definition, into numpy-like operations supported by alkaid. The tracer class needs to inherit from the abstract base class `alkaid.converter.plugin:DAISTracerPluginBase`, and implement the two abstract methods, `apply_model` and `get_input_shapes`.
...
| hls4ml    | `hls4ml`      | [alkaid_emu](https://github.com/calad0i/alkaid_emu)  | Converts hls4ml ModelGraph to DAIS, in progress and ok for experimental use |
| Torch/LGN | `torch`       | [Torch-LGN](https://github.com/calad0i/alkaid-torch) | Minimal work-in-progress plugin/skeleton                                    |

The [HGQ2](https://github.com/calad0i/HGQ2) package is the primary real-world plugin for alkaid. It registers a tracer under the `keras` key in the `dais_tracer.plugins` entry-point group, enabling `trace_model(keras_model)` to automatically convert HGQ2/Keras3 models. Installing HGQ2 is sufficient — no additional configuration is needed.

## Example Plugin

An example plugin class is provided in the `alkaid.converter.example` module for demonstration purpose. The entry point is also defined in the `pyproject.toml` file under the group name `entry-points."dais_tracer.plugins".alkaid = "alkaid.converter.example:ExampleDAISTracer"`. This plugin is used in the unit tests of the plugin system itself (`tests/test_plugin.py`), and one may refer to that test as an example.
